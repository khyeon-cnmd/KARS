...
class PSPP_network:
            ...
            for para_index in self.tokenized_dict[paper_index].keys():
                ...
                for sen_index in self.tokenized_dict[paper_index][para_index].keys():
                    if self.edge_type == "co-occurrence": # 문장 단위로 모든 개체들의 공존 빈도를 계산
                        ...
                        root_word_list = [(entity.split(" ")[-1], entity) for entity in entity_list]
                        ...
                        for child_token_lemma_pos_dep in self.tokenized_dict[paper_index][para_index][sen_index]:
                            ...
                            check = False
                            for root_word, entity in root_word_list:
                                if child_lemma == root_word:
                                    token = child_token
                                    child_lemma = entity
                                    check = True
                                    break
                        ...
                        for root_token in root_connected_token:
                            if root_token.dep_ == "nsubjpass" or root_token.dep_ == "dobj": # 수동태 주어 우선 -> 목적어 우선
                                ...
                                new_root, new_root_lemma, new_root_pos, new_root_dep = new_root
                                
                                check = False
                                for root_word, entity in root_word_list:
                                    if new_root_lemma == root_word:
                                        new_root_lemma = entity
                                        check = True
                                        break
                                break
                        ...
                        for other_token in root_connected_token:
                            if not other_token == root_token:
                                other_token = [token for token in self.tokenized_dict[paper_index][para_index][sen_index] if token[0] == other_token][0]
                                other_token, other_lemma, other_pos, other_dep = other_token

                                check = False
                                for root_word, entity in root_word_list:
                                    if other_lemma == root_word:
                                        other_lemma = entity
                                        check = True
                                        break

                                if not self.G.has_edge(new_root_lemma, other_lemma):
                                    self.G.add_edge(new_root_lemma, other_lemma, weight=1)
                                else:
                                    self.G.edges[new_root_lemma, other_lemma]["weight"] += 1

                        # node 빈도 추가
                        for root_word, entity in root_word_list:
                            if not entity in self.G.nodes():
                                self.G.add_node(entity, weight=1)

                            if not "weight" in self.G.nodes[entity]:
                                self.G.nodes[entity]["weight"] = 1
                            else:
                                self.G.nodes[entity]["weight"] += 1

                            # year 정보 추가
                            if str(year) not in self.G.nodes[entity]:
                                self.G.nodes[entity][str(year)] = 1
                            else:
                                self.G.nodes[entity][str(year)] += 1
    ...
    def construct_PSPP_tree(self, count_limit):
        ...
        # find max weight node
        root_node = max(self.G_copy.nodes(), key=lambda x: self.G_copy.nodes[x]["weight"])
        found_targets = [root_node]
        self.tree_G.add_node(root_node)
