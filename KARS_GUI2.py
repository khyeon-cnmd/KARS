...
class KARS:
    ...
    def US(self, email, DB_name):
        ...
        # 2. Save path
        self.save_path = f"/home1/khyeon/Researches/KARS/results/{self.user_name}/{self.DB_name}"
    ...
    def RS(self, text_type, edge_type, start_year, end_year,  modular_algorithm, modularity_resolution, modularity_seed, pagerank_filter, progress=gr.Progress()):
        # 1. Make directory for saving data
        ...
        time.sleep(1)
        research_structure(self.save_path, self.DB_name, text_type, edge_type, start_year, end_year, modular_algorithm, modularity_resolution, modularity_seed, pagerank_filter)
...
if __name__ == "__main__":
    ...
    # 2. User setting
    input_list = []
    for user in os.listdir("/home1/khyeon/Researches/KARS/results"):
        for DB in os.listdir(f"/home1/khyeon/Researches/KARS/results/{user}"):
            if os.path.exists(f"/home1/khyeon/Researches/KARS/results/{user}/{DB}/inputs.json"):
                dict = jsonlines.open(f"/home1/khyeon/Researches/KARS/results/{user}/{DB}/inputs.json").read()
    ...
    if not kars.user_name == "" and not kars.DB_name == "":
        if os.path.exists(f"/home1/khyeon/Researches/KARS/results/{kars.user_name}/{kars.DB_name}/Article_collection/inputs.json"):
                dict = jsonlines.open(f"/home1/khyeon/Researches/KARS/results/{kars.user_name}/{kars.DB_name}/Article_collection/inputs.json").read()
                input = [dict["email"], dict["DB_name"]]
    ...
    gr.TabbedInterface([user_setting, article_collection, keyword_extraction, research_structuring, trend_analyze], ["User setting","Article collection", "Keyword extraction", "Research structuring", "Trend analysis"]).queue(concurrency_count=1).launch(share=True)